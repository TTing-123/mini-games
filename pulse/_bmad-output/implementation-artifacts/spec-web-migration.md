...
**Approach:** 迁移为静态 Web 应用。Canvas 2D 负责游戏和物理，DOM 负责离散图标 HUD、第 1～4 关场内教学、颜色图例和关卡选择。保留四关、无尽模式和连锁奖励。

## Boundaries & Constraints

**Always:** 无 Godot；静态 HTML/CSS/ES Modules；Canvas 2D 游戏；DOM HUD；目标与发射数使用离散图标；第 1～4 关分别解释红蓝、紫、黄、绿机关；无尽模式实体有上限。
...
- Given 进入任一手工关卡，then 场内标注并短暂显示该关新机关
...
- 视觉采用深海实验室：深青色生物发光背景、玻璃舱 HUD，以及机关旁的直接发光标签。
- 颜色图例与 HUD 使用同一套色彩语义。
